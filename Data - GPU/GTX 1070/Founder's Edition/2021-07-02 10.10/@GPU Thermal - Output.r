...
if (length(unique(GROUPS$Test)) == 1)	GROUPS$Test	=	NULL
DATAS	=	list(
...
FPSsummary	=	function(UNIT = "ms", DATA = PresentMon)	{
	...
	out	=	rbind(out,
		...
		STATS(DATA[DATA$TimeInSeconds >= SECTS(0.9), ])
	)
...
GPUslopes	=	function(DATA = dataALL, PERIOD = TESTname)	{
	dataTEST	=	dataALL[dataALL$Period == PERIOD, ]
	SECTS	=	function(QUAN)	quantile(dataTEST$Time, QUAN)
	...
	out	=	rbind(out,
		slope(),
		slope(dataTEST[SECTS(0.01) < dataTEST$Time & dataTEST$Time <= SECTS(0.11), ]),
		slope(dataTEST[SECTS(0.89) < dataTEST$Time & dataTEST$Time <= SECTS(0.99), ])
		)
	colnames(out)	=	c("GPU_Temp",	"GPU_Clock",	"GPU_Power")
	rownames(out)	=	c("Test Period",	"1% to 11%",	"89% to 99%")
	return(out)
}
...
sinkTXT	=	function()	{
	...
	if (length(unique(longSUM$GPU)) == 1)	longSUM$GPU	=	NULL
	...
	sink(paste0(TESTname, " - Stats.txt"), split = TRUE)
		...
		if (!is.null(FREQspec))	{
			...
			ECDF	=	ecdf(ceiling(dataALL[dataALL$Period == TESTname, ]$GPU_Clock))
			BASE	=	min(FREQspec)
			
			LESS	=	ECDF(FREQspec - 0.01)				;	names(LESS)	=	paste0("<",	FREQspec,	" MHz")
			EQUA	=	diff(ECDF(c(BASE - 0.01, BASE)))	;	names(EQUA)	=	paste0("=",	BASE,		" MHz")
			
			print(round(c(LESS, EQUA)[c(1, 3, 2)] * 100, 2))
			#	assuming FREQspec is only Base and Boost frequencies, so if there are more values in it, there will be problems
		}
...
#	the change of bringing OCCHTML into writeOCC will need to be undone for the file LaTeX reads into the article
...
COLORS	=	scale_color_manual(
		name	=	NULL,
		values	=	c(Temperature = "red",	Frequency = "blue",	"GPU Power" = "green",	"VRAM Frequency" = "orange",	"Zero RPM" = "black",	"FPS"	=	"magenta")
	)
...
#	Time Series graph
frameTS	=	function(DELT = duration, DATA = "MsBetweenPresents")	{
	...
	TS	=	decompose(ts(DATAsets[[DATA]], deltat = 1/DELT), type = "additive")
	# TS.df	=	fortify(TS)	#	from ggfortify
	TS.df	=	data.frame(as.vector(time(TS$trend)) - 1, as.vector(TS$trend))	#	Index - 1 to align with Time labels
...
# frameTS()
# frameTS(duration/testLEN, "GPU_Clock")
...
FREQspec_line	=	function(FREQ	=	FREQspec)	{
	...
	FREQdata	=	list(
		Period	=	ordered(levsPER[1], levsPER),	x	=	FREQspec,	y	=	Inf,
		TEXT	=	FREQspec,
		ECDF	=	round2(ecdf(dataALL[dataALL$Period == TESTname, ]$GPU_Clock)(FREQspec))
		)
...
#Frame Time		default values are being supplied here
